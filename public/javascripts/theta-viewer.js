...
(function ($) {
    ...
    var ThetaViewer = function (element, texture, mode) {
        ...
        /*
              var that = this;
        var revFrag = navigator.userAgent.indexOf('Chrome') != -1;
                    // 傾きを取得
			...
			window.addEventListener("deviceorientation", function(evt){
				...
				if (revFrag) {
					if (window.orientation==90 && 0<lat) {
						lat -= 180;
						lng -= 180;
					} else if (window.orientation==-90 && lat<0) {
						lat += 180;
						lng += 180;
					}
				}
                ...
                //$("#msg").html(that.camera.rotation.x+" , "+(evt.beta* Math.PI / 180));
    ...
    function createCubeMapTexture(image_url, onload, onerror) {
        ...
        function buildCubeMapTextures(img) {
            ...
            /*
            canvas[2].get(0).getContext('2d').rotate(90*Math.PI/180);
            canvas[0].get(0).getContext('2d').rotate(90*Math.PI/180);
...
/*
        ...
        */if (Modernizr.csstransforms3d === true && Modernizr.canvas === true) {
